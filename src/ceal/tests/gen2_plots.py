...
if len(sys.argv) != 2:
    print "Usage:\t python " + sys.argv[0] + " [primes|tak|isort|psi|mmult|perm]"
    sys.exit(1)
if sys.argv[1] == "primes" or sys.argv[1] == "tak" or sys.argv[1] == "isort" or sys.argv[1] == "psi" or sys.argv[1] == "mmult" or sys.argv[1] == "perm":
...
for key, value in data[benchmark].items():
    for i in range(len(value)):
        if benchmark == "tak" or benchmark == "isort" or benchmark == "psi":
            value[i] /= 10000
        elif benchmark == "primes" or benchmark == "mmult":
            value[i] /= 1000
