...
if len(sys.argv) != 2:
    print "Usage:\t python " + sys.argv[0] + " [primes|tak|isort|psi|mmult|perm|dedup]"
    sys.exit(1)
if sys.argv[1] == "primes" or sys.argv[1] == "tak" or sys.argv[1] == "isort" or sys.argv[1] == "psi" or sys.argv[1] == "mmult" or sys.argv[1] == "perm" or sys.argv[1] == "dedup":
...
data = {
    ...
    'perm' :  { '64' : [0.24, 0.27, 0.4, 1.05], '128' : [0.6, 0.62, 0.71, 1.44], '256' : [1.14, 1.19, 1.55, 2.17], '512' : [3.44, 3.7, 3.87, 4.83],  '1024' : [11.97, 12.16, 12.86, 14.02] }
}
...
for key, value in data[benchmark].items():
    for i in range(len(value)):
        if benchmark == "tak" or benchmark == "isort" or benchmark == "psi":
            value[i] /= 10000
        elif benchmark == "primes" or benchmark == "mmult" or benchmark == "dedup":
            value[i] /= 1000
...
x_axis_labels = {
    ...
    'dedup' : ("Number of Array Elements ($N$)", [16, 32, 64]),
